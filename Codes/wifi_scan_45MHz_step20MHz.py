...
SAMPLE_RATE = int(45e6)   # ★ 45 MHz
RX_BW       = int(45e6)   # ★ 45 MHz
FFT_SIZE    = 2048        # ★ 提高频率分辨率
...
def capture_waterfall(freq):
    ...
    wf = np.zeros((WATERFALL_FRAMES, FFT_SIZE))

    window = np.hanning(FFT_SIZE)

    for i in range(WATERFALL_FRAMES):
        samples = sdr.rx()
        samples *= window
        spectrum = np.fft.fftshift(np.fft.fft(samples, FFT_SIZE))
        wf[i] = 20 * np.log10(np.abs(spectrum) + 1e-12)

    return wf

def display_channel(idx):
    wf = waterfall_cache[idx]
    if wf is None:
        return
    ...
    vmin = np.percentile(wf, 10)
    vmax = np.percentile(wf, 99)
    ...
    ax_wf.set_title(f"Waterfall – Wi-Fi CH{ch} (45 MHz BW)")
    ax_wf.set_xlabel("Frequency (MHz)")
    ax_wf.set_ylabel("Time")

    ax_fft.plot(freqs, wf[-1])
...
# =========================
def scan_all(event):
    print("Scanning all channels (45 MHz BW, 20 MHz step)...")
